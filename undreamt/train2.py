...
def main_train():
    ...
    training_group.add_argument('--model_path', type=str, default = "", help='Path to the saved model folder if you want to continue from initialization phase')
    training_group.add_argument('--accumulator_coefficient',type=int,help='Number of divisions of the batch')
    training_group.add_argument('--no_encoder_grad',default=False,type=bool,help='encoder gradient enable or disable(false means gradient enable)')
    # Optimization
    ...
    optimization_group.add_argument('--summarization_iterations', type=int, default=300000, help='the number of training iterations for summarization phase (defaults to 300000)')
    ...
    if args.learn_encoder_embeddings:
        ...
        if not args.no_encoder_grad:
            add_optimizer(src_encoder_embeddings,'src_encoder_embeddings', (src2src_optimizers, src2trg_optimizers,el2hs_optimizers,el2es_optimizers))
        else:
            add_optimizer(src_encoder_embeddings,'src_encoder_embeddings', (src2src_optimizers, src2trg_optimizers))
        add_optimizer(trg_encoder_embeddings, 'trg_encoder_embeddings', (trg2trg_optimizers, trg2src_optimizers))
    else:
    ...
    if args.fixed_decoder_embeddings:
        ...
        add_optimizer(src_decoder_embeddings, (src2src_optimizers, 'src_decoder_embeddings', trg2src_optimizers, el2es_optimizers))
        add_optimizer(trg_decoder_embeddings, (trg2trg_optimizers, 'trg_decoder_embeddings', src2trg_optimizers, el2hs_optimizers))
    if args.fixed_generator:
        ...
        add_optimizer(src_embedding_generator, 'src_embedding_generator', (src2src_optimizers, trg2src_optimizers, el2es_optimizers))
        add_optimizer(trg_embedding_generator, 'trg_embedding_generator', (trg2trg_optimizers, src2trg_optimizers, el2hs_optimizers))
        ...
        add_optimizer(src_generator,'src2src_optimizers', (src2src_optimizers, trg2src_optimizers,el2es_optimizers))
        add_optimizer(trg_generator, 'trg2trg_optimizers', (trg2trg_optimizers, src2trg_optimizers,el2hs_optimizers))
    ...
    if not args.no_encoder_grad:
        add_optimizer(encoder, 'encoder', (src2src_optimizers, trg2trg_optimizers, src2trg_optimizers, trg2src_optimizers,el2hs_optimizers,el2es_optimizers))
    else:
        add_optimizer(encoder, 'encoder', (src2src_optimizers, trg2trg_optimizers, src2trg_optimizers, trg2src_optimizers))
    # Build discriminator
    ...
    add_optimizer(discriminator, 'discriminator', (src2src_optimizers, trg2trg_optimizers, src2trg_optimizers, trg2src_optimizers))
    # Build decoders
    ...
    add_optimizer(src_decoder, 'src_decoder', (src2src_optimizers, trg2src_optimizers,el2es_optimizers))#
    add_optimizer(trg_decoder, 'trg_decoder', (trg2trg_optimizers, src2trg_optimizers,el2hs_optimizers))#

    if args.phase == 2 and args.model_path != "":
        dir=os.listdir(args.model_path)
        models = ['encoder','discriminator','src_decoder','trg_decoder','src_encoder_embeddings','src_decoder_embeddings','src_generator','trg_encoder_embeddings','trg_decoder_embeddings','trg_generator','src_dictionary','trg_dictionary']
        path = {}
        for model in models:
        ...
        src_decoder=torch.load(os.path.join(args.model_path,path["src_decoder"]))
        trg_decoder=torch.load(os.path.join(args.model_path,path["trg_decoder"]))
        ...
        src_dictionary=torch.load(os.path.join(args.model_path,path["src_dictionary"]))
        trg_dictionary=torch.load(os.path.join(args.model_path,path["trg_dictionary"]))
    ...
    el2es_summarizer = Translator(encoder_embeddings=src_encoder_embeddings,
                                    decoder_embeddings=src_decoder_embeddings, generator=src_generator,
                                    src_dictionary=src_dictionary, trg_dictionary=src_dictionary, encoder=encoder,
                                    decoder=src_decoder, denoising=False, device=device,rec=False,discriminator=None,source=1,no_encoder_grad=args.no_encoder_grad)#
    el2hs_summarizer = Translator(encoder_embeddings=src_encoder_embeddings,
                                    decoder_embeddings=trg_decoder_embeddings, generator=trg_generator,
                                    src_dictionary=src_dictionary, trg_dictionary=trg_dictionary, encoder=encoder,
                                    decoder=trg_decoder, denoising=False, device=device,rec=False,discriminator=None,source=1,no_encoder_grad=args.no_encoder_grad)#
    ...
    if args.summary is not None:
        ...
        el2es_trainer = Trainer(translator=el2es_summarizer, optimizers=el2es_optimizers, corpus=corpus, batch_size=args.batch,summary = True,accumulator_coefficient=args.accumulator_coefficient)
        ...
        if not args.disable_backtranslation:
            el2hs_trainer = Trainer(translator=el2hs_summarizer, optimizers=el2hs_optimizers,
                                          corpus=data.BacktranslatorCorpusReader(corpus=corpus, translator=src2trg_translator,summary = True), batch_size=args.batch,summary=True,accumulator_coefficient=args.accumulator_coefficient)
            trainers_summarization.append(el2hs_trainer)        ## check again
        #f1 = open(args.summary[0], encoding=args.encoding, errors='surrogateescape')
        f2 = open(args.summary[1], encoding=args.encoding, errors='surrogateescape')
        corpus = data.CorpusReader(f2, max_src_sentence_length=args.max_sentence_length, max_trg_sentence_length=args.max_sentence_length, cache_size=args.cache)
        s_trgback2src_trainer = Trainer(translator=trg2src_translator, optimizers=trg2src_optimizers,
                                      corpus=data.BacktranslatorCorpusReader(corpus=corpus, translator=src2trg_translator), batch_size=args.batch)
        trainers_summarization.append(s_trgback2src_trainer)
    if args.src2trg is not None:
        ...
        trainers_initialization.append(src2trg_trainer) # to be removed
    ...
    if args.trg2src is not None:
        ...
        trainers_initialization.append(trg2src_trainer) #to be removed
    ...
    loggers_summarization.append(Logger('Target to source (backtranslation summary)', s_trgback2src_trainer, args.log_interval, [], None, args.encoding))
    ...
    def save_models(name ,initialization = True):
        if initialization:
            torch.save(src2src_translator, '{0}.{1}.src2src.pth'.format(name, args.save))
            torch.save(trg2trg_translator, '{0}.{1}.trg2trg.pth'.format(name, args.save))
            torch.save(src2trg_translator, '{0}.{1}.src2trg.pth'.format(name, args.save))
            torch.save(trg2src_translator, '{0}.{1}.trg2src.pth'.format(name, args.save))
            torch.save(encoder,'{0}.{1}.encoder.pth'.format(name, args.save))
            torch.save(discriminator,'{0}.{1}.discriminator.pth'.format(name, args.save))
            torch.save(src_decoder,'{0}.{1}.src_decoder.pth'.format(name, args.save))
            torch.save(trg_decoder,'{0}.{1}.trg_decoder.pth'.format(name, args.save))
            torch.save(src_encoder_embeddings,'{0}.{1}.src_encoder_embeddings.pth'.format(name, args.save))
            torch.save(src_decoder_embeddings,'{0}.{1}.src_decoder_embeddings.pth'.format(name, args.save))
            torch.save(src_generator,'{0}.{1}.src_generator.pth'.format(name, args.save))
            torch.save(trg_encoder_embeddings,'{0}.{1}.trg_encoder_embeddings.pth'.format(name, args.save))
            torch.save(trg_decoder_embeddings,'{0}.{1}.trg_decoder_embeddings.pth'.format(name, args.save))
            torch.save(trg_generator,'{0}.{1}.trg_generator.pth'.format(name, args.save))
            torch.save(src_dictionary,'{0}.{1}.src_dictionary.pth'.format(name, args.save))
            torch.save(trg_dictionary,'{0}.{1}.trg_dictionary.pth'.format(name, args.save))


        else:
            torch.save(src2src_translator, '{0}.{1}.s_src2src.pth'.format(name, args.save))
            torch.save(trg2trg_translator, '{0}.{1}.s_trg2trg.pth'.format(name, args.save))
            torch.save(src2trg_translator, '{0}.{1}.s_src2trg.pth'.format(name, args.save))
            torch.save(trg2src_translator, '{0}.{1}.s_trg2src.pth'.format(name, args.save))
            torch.save(el2es_summarizer, '{0}.{1}.el2es.pth'.format(name, args.save))#
            torch.save(el2hs_summarizer, '{0}.{1}.el2hs.pth'.format(name, args.save))#
            torch.save(encoder,'{0}.{1}.s_encoder.pth'.format(name, args.save))
            torch.save(discriminator,'{0}.{1}.s_discriminator.pth'.format(name, args.save))
            torch.save(src_decoder,'{0}.{1}.src_decoder.pth'.format(name, args.save))
            torch.save(trg_decoder,'{0}.{1}.trg_decoder.pth'.format(name, args.save))
            torch.save(src_encoder_embeddings,'{0}.{1}.s_src_encoder_embeddings.pth'.format(name, args.save))
            torch.save(src_decoder_embeddings,'{0}.{1}.s_src_decoder_embeddings.pth'.format(name, args.save))
            torch.save(src_generator,'{0}.{1}.s_src_generator.pth'.format(name, args.save))
            torch.save(trg_encoder_embeddings,'{0}.{1}.s_trg_encoder_embeddings.pth'.format(name, args.save))
            torch.save(trg_decoder_embeddings,'{0}.{1}.s_trg_decoder_embeddings.pth'.format(name, args.save))
            torch.save(trg_generator,'{0}.{1}.s_trg_generator.pth'.format(name, args.save))
    ...
    def initialization_training(l):
            ...
            save_models('{0}final_UNMT'.format(args.model_path))#
        #summarization phase
    def summarization_training(l):
        loggers = l
        for step in range(1, args.summarization_iterations + 1):#
        ...
        save_models('{0}final_NMT_SUMMARIZATION'.format(args.model_path) ,initialization=False)#
    if args.phase == 1:#
...
class Trainer:
    def __init__(self, corpus, optimizers, translator,accumulator_coefficient=None, batch_size=50, summary = False):
        self.corpus = corpus
        self.translator = translator
        self.optimizers = optimizers
        self.batch_size = batch_size
        self.summary = summary
        self.accumulator_coefficient = accumulator_coefficient
        self.reset_stats()
        self.count = 0
    def step(self):
        # Reset gradients
        ...
        t = time.time()
        datas = self.corpus.next_batch(self.batch_size)
        src, trg = datas[0], datas[1]
        ...
        # Compute loss
        t = time.time()
        """if self.accumulator_coefficient != None:
            divider = int(self.batch_size/self.accumulator_coefficient)
            if not self.summary:
                loss = [0]*3

                for i in range(0,self.accumulator_coefficient):
                    acc_loss = self.translator.score(src[divider*i:divider*(i+1)], trg[divider*i:divider*(i+1)], train=True)
                    loss[0] += acc_loss[0]
                    loss[1] += acc_loss[1]
                    loss[2] += acc_loss[2]
            else :
                ...
                for i in range(0,self.accumulator_coefficient):
                    lossk = self.translator.score(src[divider*i:divider*(i+1)], trg[divider*i:divider*(i+1)], train=True)
                    lossk.div(self.batch_size).backward()
                    loss+=lossk
                    print("summary_mode:- {0}".format(i))
                    tot_m, used_m, free_m = map(int, os.popen('free -t -m').readlines()[-1].split()[1:])
                    print("model",tot_m,used_m,free_m)
        else:
        ...
        if self.count%50 == 0:
            if self.summary:
                if len(datas)==2:
                    print("Translations :- \ninput:- {0},\n output:- {1}\n actual output:-{2}".format(src[0:2],self.translator.greedy(src[0:2], train=False,rec=False),trg[0:2]))
                else:
                    print("Translations :- \ninput:- {0},\n output:-{1}\n  hIN out actual:- {2}\n ENG out:- {3}".format(src[0:2],self.translator.greedy(src[0:2], train=False,rec=False),trg[0:2],datas[3][0:2]))
            else:
                if len(datas) ==2:
                    print("Translations :- \ninput:- {0},\n output:- {1}".format(src[0:2],self.translator.greedy(src[0:2], train=False)))
                else:
                    print("Translations :- \ninput:- {0},\n output:-{1}\n actual input:- {2}".format(src[0:2],self.translator.greedy(src[0:2], train=False),datas[2][0:2]))
    ...
    def log(self, step=0):
        ...
        if self.trainer is not None:
            if not self.trainer.summary :
                ...
                #print("log:-",self.trainer.words_per_second())
                lossEG = self.trainer.perplexity_per_word()
                print('  - Training:   {0:10.2f}  ({1:.2f}s: {2:.2f}tok/s src, {3:.2f}tok/s trg; epoch {4}),'.format(lossEG ,self.trainer.total_time(),
                    self.trainer.words_per_second()[0], self.trainer.words_per_second()[1], self.trainer.corpus.epoch))  # have to log generator and discriminator loss also
                self.trainer.reset_stats()
