...
SPECIAL_SYMBOLS = 5
PAD, OOV, EOS, REC,SUMMARY = 0, 1, 2, 3, 4    #REC=start reconstruction in backtranslation and denoising
...
class Dictionary:
    ...
    def sentence2ids(self, sentence, eos=False, rec=False,summary=False):
    ...
    def sentences2ids(self, sentences, eos=False, rec=False,summary=False):
        ids = [self.sentence2ids(sentence, eos=eos, rec=rec,summary=summary) for sentence in sentences]
    ...
    def ids2sentence(self, ids):
        return ' '.join(['<OOV>' if i == OOV else self.id2word[i - SPECIAL_SYMBOLS + 1] for i in ids if i != EOS and i != PAD and i != REC and i !=SUMMARY])
...
class CorpusReader:
    def __init__(self, src_file, trg_file=None, max_src_sentence_length=80,max_trg_sentence_length=80, cache_size=1000):
        ...
        self.max_trg_sentence_length = max_trg_sentence_length
    ...
    def _fill_cache(self):
        ...
        while len(self.cache) < self.cache_size:
            ...
            elif 0 < src_length <= self.max_src_sentence_length and 0 < trg_length <= self.max_trg_sentence_length:
...
class BacktranslatorCorpusReader:
    def __init__(self, corpus, translator,summary=False):
        self.corpus = corpus
    ...
    def next_batch(self, size):
        ...
        original_trg = trg
        if self.summary:
            trg = self.translator.greedy(trg, train=False)#
        else:
            src = self.translator.greedy(trg, train=False)#
        self.epoch = self.corpus.epoch
        return src, trg, original_src, original_trg
