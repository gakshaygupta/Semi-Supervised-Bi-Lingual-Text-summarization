#Reused code form Mikel Artetxe <artetxem@gmail.com>
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, ids, lengths, word_embeddings, hidden, no_grad = False):
        ...
        if no_grad:
            with torch.no_grad():
                return self.sudo_forward(ids, lengths, word_embeddings, hidden)

        return self.sudo_forward(ids, lengths, word_embeddings, hidden)    #[seq_len,batch,hidden_size]

    def initial_hidden(self, batch_size):
        return Variable(torch.zeros(self.layers*self.directions, batch_size, self.hidden_size), requires_grad=False)

    def sudo_forward(self, ids, lengths, word_embeddings, hidden):
        sorted_lengths = sorted(lengths, reverse=True)
        is_sorted = sorted_lengths == lengths
        is_varlen = sorted_lengths[0] != sorted_lengths[-1]
        if not is_sorted:
